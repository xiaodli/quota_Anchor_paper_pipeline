...
plot_total <- function (file, out_file, x_label){
  ...
  custom_colors = rev(c(shared_gene_pairs = "#B7E4C7", quota_Anchor = "#F3BDA5", WGDI = "#7FBEEB", MCScanX ="#D8B4E2"))
  plot = ggplot(df, aes(y=factor(comparison,levels =rev(unique(comparison))), x=number, fill=factor(types,levels = c("MCScanX", "WGDI", "quota_Anchor","shared_gene_pairs")), )) +
    ...
    scale_fill_manual(values = custom_colors,  breaks = c("shared_gene_pairs", "quota_Anchor", "WGDI", "MCScanX")) + 
