...
plot_total <- function (query_file_path, ref_file_path, col_file, fig_file, left_label, right_label){
  ...
  custom_colors <- setNames(
    c("#E0E0E0", "#0000FF", "#FF0000", "#00BA38"),         
    c("homo", right_label, left_label, "shared_gene_pairs") 
  )
  custom_alpha <- setNames(
    c(0.1, 0.5, 0.5, 0.5),                          
    c("homo", right_label, left_label, "shared_gene_pairs")
  )
  custom_size <- setNames(
    c(0.02, 0.1, 0.1, 0.1),                               
    c("homo", right_label, left_label, "shared_gene_pairs")
  )
  plot <- ggplot(data=data, aes(x=query_index, y=ref_index))+
    ...
    labs(x="Zea mays", y="Zea mays")+scale_x_continuous(expand=c(0, 0)) + scale_y_continuous(expand=c(0, 0)) +
...
plot_total("../zm.length.txt",
           "../zm.length.txt",
           "difference.quota_Anchor.MCScanX.sorted.txt",
           "difference.quota_Anchor.MCScanX.sorted.pdf",
           "quota_Anchor",
           "MCScanX")

plot_total("../zm.length.txt",
           "../zm.length.txt",
           "difference.quota_Anchor.WGDI.sorted.txt",
           "difference.quota_Anchor.WGDI.sorted.pdf",
           "quota_Anchor",
           "WGDI")

plot_total("../zm.length.txt",
           "../zm.length.txt",
           "difference.WGDI.MCScanX.sorted.txt",
           "difference.WGDI.MCScanX.sorted.pdf",
           "WGDI",
           "MCScanX")
