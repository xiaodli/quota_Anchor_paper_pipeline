...
def get_wgd_pairs(file):
    sub_df = pd.read_csv(file, index_col=None, header=0, sep="\t", comment="#")
    pair_map = {}
    for i, row in sub_df.iterrows():
        pair_map[row.iloc[0] + "\t" + row.iloc[5]] = "shared_gene_pairs"
        pair_map[row.iloc[5] + "\t" + row.iloc[0]] = "shared_gene_pairs"
    return pair_map

def MCScanX_get_pairs(file):
    sub_df = pd.read_csv(file, index_col=None, header=None, sep="\t", comment="#")
    pair_map = {}
    for i, row in sub_df.iterrows():
        pair_map[row.iloc[2] + "\t" + row.iloc[1]] = "shared_gene_pairs"
        pair_map[row.iloc[1] + "\t" + row.iloc[2]] = "shared_gene_pairs"
    return pair_map

def WGDI_get_pairs(file):
    sub_df = pd.read_csv(file, index_col=None, header=None, sep=r'\s+', comment="#")
    pair_map = {}
    for i, row in sub_df.iterrows():
        pair_map[row.iloc[2] + "\t" + row.iloc[0]] = "shared_gene_pairs"
        pair_map[row.iloc[0] + "\t" + row.iloc[2]] = "shared_gene_pairs"
    return pair_map
...
if __name__ == '__main__':
    ...
    blast_df["Type"] = "homo"
    if sys.argv[7] == "quota_Anchor":
        quota_anchor_map = get_wgd_pairs(sys.argv[4])
    if sys.argv[7] == "WGDI":
        quota_anchor_map = WGDI_get_pairs(sys.argv[4])
    if sys.argv[7] == "MCScanX":
        quota_anchor_map = MCScanX_get_pairs(sys.argv[4])
    quota_anchor_keys = quota_anchor_map.keys()

    if sys.argv[8] == "quota_Anchor":
        MCScanX_pairs_map = get_wgd_pairs(sys.argv[5])
    if sys.argv[8] == "WGDI":
        MCScanX_pairs_map = WGDI_get_pairs(sys.argv[5])
    if sys.argv[8] == "MCScanX":
        MCScanX_pairs_map = MCScanX_get_pairs(sys.argv[5])

    MCScanX_keys = MCScanX_pairs_map.keys()

    total_map = {**quota_anchor_map, **MCScanX_pairs_map}

    left_label = sys.argv[7]
    right_label = sys.argv[8]
    quota_Anchor_minus_MCScanX = list(set(quota_anchor_keys) - set(MCScanX_keys))
    for i in quota_Anchor_minus_MCScanX:
        total_map[i] = left_label

    MCScanX_minus_quota_Anchor = list(set(MCScanX_keys) - set(quota_anchor_keys))
    for j in MCScanX_minus_quota_Anchor:
        total_map[j] = right_label
