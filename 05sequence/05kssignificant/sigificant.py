...
def main(parameter):
    ...
    raw_same, raw_inv= split_inv_normal_to_pair(df)
    sign = 0
    loop_number = 10000
    for _ in range(loop_number):
        same = random.choices(raw_same, k=50000)
        inv = random.choices(raw_inv, k=10000)
        ...
        res = scipy.stats.mannwhitneyu(recent_same_ks, recent_inv_ks ,alternative="less")
        p_value = res.pvalue
        if p_value < 0.05:
            sign += 1
        # print(np.mean(recent_same_ks)- np.mean(recent_inv_ks))
        #
    ...
    non_sign = loop_number-sign
    sign_ratio = round((sign/loop_number * 100), 2)
    non_sign_ratio = round(100 - sign_ratio, 2)

    with open (output, "a+") as f:
        if os.path.getsize(output) == 0:
            f.write("Species,Type,Number,Ratio\n")
        f.write(str(species_name) + "," + "Significant" + "," + str(sign) + "," + f"{sign_ratio:.2f}" + "\n" )
        f.write(str(species_name) + "," + "Non-significant" + "," + str(non_sign) + "," + f"{non_sign_ratio:.2f}" + "\n")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ks significant number')

    subparsers1 = parser.add_subparsers(title='ks significant number', dest='ks_sign')
    parser_sub1 = subparsers1.add_parser('ks_sign', help='ks significant number')
