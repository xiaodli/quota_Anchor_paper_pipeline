...
if [[ "$newname" != "Joinvillea.ascendens.pep" ]]; then
     quota_Anchor pre_col -a diamond -rs $peppath/Joinvillea.ascendens.pep -qs $peppath/$newname -db Joinvillea.ascendens.diamond -mts 20 -e 1e-10 -b $colpath/${newname%.pep}/Joinvillea.ascendens.${newname%.pep}.blast -rg $gffpath/Joinvillea.ascendens.gff3 -qg $gffpath/${newname%.pep}.gff3 -o $colpath/${newname%.pep}/Joinvillea.ascendens.${newname%.pep}.table -bs 100 -al 0 -rl $gffpath/Joinvillea.ascendens.length.txt -ql $gffpath/${newname%.pep}.length.txt --overwrite
     quota_Anchor col -i $colpath/${newname%.pep}/Joinvillea.ascendens.${newname%.pep}.table -o $colpath/${newname%.pep}/Joinvillea.ascendens.${newname%.pep}.collinearity -s 0 --overwrite -D 25 -a 0 -r $number -q 1 -W 5 -m 500 -I 5
