setwd("/media/dell/E/Suppmentary_data/02Spontaneum_P.hallii_.C.macrourus_E.colona/difference_stats/")
library(ggplot2)
library(viridis)
library(ggpubr)
...
plot_total_pdf <- function (file, out_file, parameter3, parameter4, parameter5=3.3){
  ...
  data$types = factor(data$types, levels(data$types) <- c("SOI", "WGDI", "quota_align", "quota_Anchor","shared_gene_pairs"))
  custom_colors_light <- c(shared_gene_pairs = "#B7E4C7", quota_Anchor = "#F3BDA5", quota_align = "#7FBEEB", WGDI ="#D8B4E2", SOI = "#FDFD96")
  custom_colors_dark <- c(shared_gene_pairs = "#B7E4C7", quota_Anchor = "#F3BDA5", quota_align = "#7FBEEB", WGDI ="#D8B4E2", SOI = "#FDFD96")
  print(data)
  ...
  plot <- ggplot(data, aes(x="", y=ratio, fill=types, color=types)) + 
    ...
    geom_text(aes(label=paste0(ratio, "%")), position=position_stack(vjust=0.6), size = 5, color="#F8766D")+
    ...
    theme(axis.title.x=element_blank(),
          ...
          legend.title = element_text(size = 23, margin = margin(r = 0.6, unit = "cm")),
...
plot_total_pdf("./cm.bar_difference_R.txt", "./cm.pie.pdf", "Identity", "identity", 1.3)
plot_total_pdf("./ec.bar_difference_R.txt", "./ec.pie.pdf", "Identity", "identity", 1.3)
plot_total_pdf("./ph.bar_difference_R.txt", "./ph.pie.pdf", "Identity", "identity", 1.3)