setwd("/media/dell/E/Suppmentary_data/02Spontaneum_P.hallii_.C.macrourus_E.colona/difference/")
library(ggplot2)
# changetoM <- function ( position ){
#   position=position/1000000;
#   paste(position, "M", sep="")
# }
plot_total <- function (query_file_path, ref_file_path, col_file, fig_file, query_name, ref_name, angle){
  ...
  custom_alpha <- c(homo = 0.1, quota_align =0.5, quota_Anchor=0.5, shared_gene_pairs=0.5)
  ...
  plot <- ggplot(data=data, aes(x=query_index, y=ref_index))+
    ...
    geom_point(size=0.01, aes(color=Type, alpha = Type)) +
    geom_blank(data=blank_df, aes(x=queryLength, y=refLength)) +
    scale_color_manual(values = custom_colors) +
    scale_alpha_manual(values = custom_alpha) +
    theme_grey(base_size = 30) +
    labs(x=query_name, y=ref_name)+scale_x_continuous(expand=c(0, 0)) + scale_y_continuous(expand=c(0, 0)) +
    theme(axis.line = element_blank(),
          panel.spacing = unit(0, "mm"),
          # strip.background = element_rect(color = "#a0a0a0"),
          strip.background = element_blank(),
          strip.text.x = element_text(size = 16, angle=angle, hjust=0.3, vjust=.5),
          strip.text.y = element_text(size = 13, angle=0, hjust=0, vjust=0.5),
...
plot_total("/media/dell/E/Suppmentary_data/05sequence/01pipeline_raw_data/Panicum.hallii.length.txt",
           "/media/dell/E/Suppmentary_data/05sequence/01pipeline_raw_data/Saccharum.spontaneum.length.txt",
           "./quota_align_Panicum.txt",
           "./quota_align_Panicum.hallii_diamond_difference.pdf",
           "Panicum hallii",
           "Saccharum spontaneum",
           0)
plot_total("Cenchrus.macrourus.length.txt",
           "/media/dell/E/Suppmentary_data/05sequence/01pipeline_raw_data/Saccharum.spontaneum.length.txt",
           "./rm_GWHBWDX000000_quota_align_Cenchrus.txt",
           "./quota_align_Cenchrus.macrourus_diamond_difference.pdf",
           "Cenchrus macrourus",
           "Saccharum spontaneum",
           0)
plot_total("/media/dell/E/Suppmentary_data/05sequence/01pipeline_raw_data/Echinochloa.colona.length.txt",
           "/media/dell/E/Suppmentary_data/05sequence/01pipeline_raw_data/Saccharum.spontaneum.length.txt",
           "./quota_align_Echinochloa.txt",
           "./quota_align_Echinochloa.colona_diamond_difference.pdf",
           "Echinochloa colona",
           "Saccharum spontaneum",
           60)
