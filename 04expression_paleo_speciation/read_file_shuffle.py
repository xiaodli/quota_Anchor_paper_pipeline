...
def read_three_file(table_file, col_file1, collinearity_total ,length, query_expr_dict, ref_expr_dict, bootstrap, size, method, corr):
    df_list = []
    ...
    speciation_list_ele_tuple = process_file.get_corr_data(query_expr_dict, ref_expr_dict, speciation, bootstrap, size)
    speciation_dict_corr = corr_analysis.size_corr(speciation_list_ele_tuple, method)
    corr_df3 = pd.DataFrame(list(speciation_dict_corr.items()), columns=["Index3", "Corr_Value3"])
    df_list.append(corr_df3)

    # get paleo syntenic gene pairs
    _, total_info = read_collinearity(collinearity_total, length)
    ...
    same, inv = process_file.split_inv_normal_to_pair(total_df)
    total = list(set(same) | set(inv))
    # Note: This is a rough division
    paleo = list(set(total) - set(speciation))
    paleo_list_ele_tuple = process_file.get_corr_data(query_expr_dict, ref_expr_dict, paleo, bootstrap, size)
    paleo_dict_corr = corr_analysis.size_corr(paleo_list_ele_tuple, method)
    corr_df4 = pd.DataFrame(list(paleo_dict_corr.items()), columns=["Index4", "Corr_Value4"])
    df_list.append(corr_df4)

    # blast gene pairs(non conserved gene order)
    blast_table = pd.read_csv(table_file, header=None, sep="\t", index_col=None)
    blast_table.columns = ["ref_name", "ref_chr", "ref_order", "ref_start", "ref_end", "ref_strand",
                                             "query_name", "query_chr", "query_order", "query_start", "query_end",
                                             "query_strand", "score"]
    blast_pairs = list(zip(blast_table["query_name"].values.tolist(), blast_table["ref_name"].values.tolist()))
    filtered_pairs = list(set(blast_pairs) - set(speciation) - set(paleo))
    filtered_pairs_list_ele_tuple = process_file.get_corr_data(query_expr_dict, ref_expr_dict, filtered_pairs, bootstrap, size)
    filtered_pairs_dict_corr = corr_analysis.size_corr(filtered_pairs_list_ele_tuple, method)
    corr_df5 = pd.DataFrame(list(filtered_pairs_dict_corr.items()), columns=["Index5", "Corr_Value5"])
    df_list.append(corr_df5)
