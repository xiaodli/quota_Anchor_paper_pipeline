...
library(grid)
library(ggpattern)
df = read.table("/media/dell/E/Suppmentary_data/04expression_paleo_speciation/speciation.paleo.ssd.sid.R.txt", header = TRUE, check.names = FALSE, sep = ",")
head(df)
...
df$Species <- factor(df$Species, levels = c("sorghum vs wheat", "sorghum vs maize", "maize vs wheat"))
...
plot = ggplot(df, aes(x=Stage, y=Ratio, fill = Stage)) +
  ...
  theme(axis.title.x=element_blank(),          
        ...
        strip.text = element_text(size = 30, angle=0, margin = margin(c(0,0,30,0)), colour = "black") ,
...
pdf("/media/dell/E/Suppmentary_data/04expression_paleo_speciation/speciation.paleo.ssd.sid.R.pdf", width=16, height=10)
print(plot)
dev.off()
