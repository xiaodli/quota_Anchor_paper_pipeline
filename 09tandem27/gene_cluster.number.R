...
plot = ggplot(df, aes(x=Species, y=tandem_total_number)) +
...
plot = ggplot(df, aes(x=Species, y=Cluster_length_max)) +
  labs(x="", y="Max cluster lengeh \n(max proximal distance = 5)") +  
  scale_y_continuous(expand=c(0, 0)) +
  
  geom_line(aes(x = Species, y = Cluster_length_max, color = Depth, group = Depth), linewidth = 1, alpha = 1) +
  geom_point(aes(x = Species, y = Cluster_length_max, color = Depth, group = Depth), size = 3, alpha = 1) +
  
  scale_alpha_manual(values = custom_alpha) +
  scale_color_manual(values = custom_colors)+
  
  guides(
    color = guide_legend(title = "Max cluster lengeh (Depth)", override.aes = list(alpha = NA), order = 2),
    alpha = "none"
  )+
