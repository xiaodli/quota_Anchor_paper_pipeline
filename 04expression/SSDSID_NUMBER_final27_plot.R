...
setwd("/media/dell/E/Suppmentary_data/04expression/")
...
print(data)
data$Species <- factor(data$Species, c("sorghum vs wheat", "sorghum vs maize", "maize vs wheat"))

plot <- ggplot(data, aes(x="", y=Ratio, fill=Type, color=Type)) + 
  facet_wrap(~Species, nrow = 1, ncol = 3, scales = "fixed",
             shrink = FALSE, as.table = TRUE, drop = TRUE) +
  geom_bar(width = 0.01, stat = "identity", linewidth=0.1) +
  coord_polar("y", start = 0)+
  geom_text(aes(label=paste0(Number, "\n", "(",  Ratio, "%)")), position=position_stack(vjust=0.5), size = 7.5, color="#F8766D")+
...
plot_total_pdf("/media/dell/E/Suppmentary_data/04expression/three.ssd.sid.R.txt", "/media/dell/E/Suppmentary_data/04expression/three.ssd.sid.R.pdf", "Identity", "identity", 1.3)
