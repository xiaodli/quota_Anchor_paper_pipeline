...
for sheet_prefix_index in "${!sheet_prefix_array[@]}"; do {
  ...
  for folder_index in "${!folder_array[@]}"; do {
          ...
          python main.py bootstrap_corr -W "$collinearity"  -e all_special_tpm.xlsx -m pearson -c "${column}" -b T -B 5 -s "$shoot_or_root" -boot 50000 -sample_size 30 -co ${out_path1}/${column}_pearson_correlation_log.csv -sheet_prefix $sheet_prefix
          python main.py process -i ${out_path1}/${column}_pearson_correlation_log.csv -o ${out_path2}/${column}_pearson_correlation_log.R.csv

          python main.py bootstrap_corr -W "$collinearity"  -e all_special_tpm.xlsx -m spearman -c "${column}" -b T -B 5 -s "$shoot_or_root" -boot 50000 -sample_size 30 -co ${out_path1}/${column}_spearman_correlation_log.csv -sheet_prefix $sheet_prefix
          ...
          python main.py bootstrap_corr -W "$collinearity"  -e all_special_tpm.xlsx -m pearson -c "${column}" -b F -B 5 -s "$shoot_or_root" -boot 50000 -sample_size 30 -co ${out_path1}/${column}_pearson_correlation.csv -sheet_prefix $sheet_prefix
          python main.py process -i ${out_path1}/${column}_pearson_correlation.csv -o ${out_path2}/${column}_pearson_correlation.R.csv

          python main.py bootstrap_corr -W "$collinearity"  -e all_special_tpm.xlsx -m spearman -c "${column}" -b F -B 5 -s "$shoot_or_root" -boot 50000 -sample_size 30 -co ${out_path1}/${column}_spearman_correlation.csv -sheet_prefix $sheet_prefix
