...
plot <- ggplot(data, aes(x=Type, y=Corr_Value, fill=Type, color=Type)) + 
  ...
  geom_boxplot(position = position_dodge(width = 0.01),width=1, alpha=1, size=0.1, outliers = FALSE,  staplewidth = 0.3) +
  stat_compare_means(label = "p.signif", size=1.5, comparisons=my_comparisons,  tip.length = 0.01, bracket.size = 0.04, step.increase = 0.08, hide.ns = FALSE, method = "wilcox.test", method.args = list(alternative = "greater"))  + 
...
plot_total("/media/dell/E/Suppmentary_data/04expression/pearson_correlation_log.csv", "/media/dell/E/Suppmentary_data/04expression/pearson_correlation_log.png", "pearson")
plot_total("/media/dell/E/Suppmentary_data/04expression/spearman_correlation.csv", "/media/dell/E/Suppmentary_data/04expression/spearman_correlation.png", "pearson")
